pipeline {
    ...
    stages {
        ...
        stage ('Build RPM') {
            ...
            steps {
                script {
                    sh "sed -i 's/^Version:.*/Version: ${determineRpmVersion()}/' rpm/ingrid-codelist-repository.spec"
                    sh "sed -i 's/^Release:.*/Release: ${determineRpmReleasePart()}/' rpm/ingrid-codelist-repository.spec"

                    // Prepare build
                    sh "mkdir -p ./target/rpms /root/rpmbuild/SPECS"
                    sh """
                        cp ${WORKSPACE}/rpm/ingrid-codelist-repository.spec /root/rpmbuild/SPECS/ingrid-codelist-repository.spec &&
                        rpmbuild -bb /root/rpmbuild/SPECS/ingrid-codelist-repository.spec
                    """
                    ...
                    withCredentials([
                        ...
                        archiveArtifacts artifacts: 'target/rpms/ingrid/ingrid-codelist-repository-*.rpm', fingerprint: true
                    }
                    ...
                    withCredentials([
                        ...
                        archiveArtifacts artifacts: 'target/rpms/itzbund/ingrid-codelist-repository-*.rpm', fingerprint: true
                    }
                }
            }
        }
        ...
        stage('Deploy RPM') {
            ...
            steps {
                script {
                    ...
                    sh "mv target/bom.json target/ingrid-codelist-repository-${determineRpmVersion()}.bom.json"
