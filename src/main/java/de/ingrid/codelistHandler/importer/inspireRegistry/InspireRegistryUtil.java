...
public class InspireRegistryUtil {
    ...
    public CodeList importFromRegistry(URL urlGerman, URL urlEnglish) throws Exception {
        ...
        try {
            inspireCodelist = objectMapper.readValue(urlGerman.openStream(), InspireCodelistModel.class);
        } catch (Exception e) {
            log.error("Error parsing JSON data from " + urlGerman, e);
            throw e;
        }
        try {
            inspireCodelistEn = objectMapper.readValue(urlEnglish.openStream(), InspireCodelistModel.class);
        } catch (Exception e) {
